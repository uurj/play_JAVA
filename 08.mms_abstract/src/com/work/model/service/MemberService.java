...
public class MemberService extends MemberServiceAbstract {
	...
	public void addMember(Member dto) {
		...
		members[count++] = dto;  
		// 무조건 등록하니 발생하는 문제 : 중복 ==> 해결법 : 등록전에 아이디가 같은 객체가 있는지
		} else {
			System.out.println("[오류] 아이디는 사용할 수 없습니다.");
		// 동일아이디를 갖는 회원객체 존재하는 경우에는 오류메세지 출력
		// 오류메세지 : [오류] 000아이디는 사용할 수 없습니다. 
		}
	}
...
//		String memberID = null;		// (전체) 에러 솔루션 사용
//		int idx = exist(memberID);	// (매개변수) 에러 솔루션 사용
//		if (idx >= 0) {
//			return members[idx];
//		}
//		System.out.println("[오류] 전체조회를 실행할 수 없습니다.");
		return null;
	}
